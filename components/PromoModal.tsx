...
export default function PromoModal({
  ...
  const fetchPromo = useCallback(async () => {
    ...
    let data: PromotionRow | null = null
    ...
    if (d1) {
      ...
      if (allPromos) {
        data =
          (allPromos as PromotionRow[]).find((p) => {
    ...
    setLoading(false)
  }, [restroom.id, restroom.name, sessionId, onComplete])

  useEffect(() => {
    // Mount-time network fetch; setState happens asynchronously inside fetchPromo.
    // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional data load on open
    void fetchPromo()
  }, [fetchPromo])
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 60, display: 'flex', alignItems: 'flex-end' }}>
      <div style={{ background: 'white', borderRadius: '20px 20px 0 0', padding: '28px 24px', width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <div
            style={{
              background: seconds > 0 ? '#F59E0B' : '#1D9E75',
              color: 'white',
              borderRadius: '20px',
              padding: '4px 12px',
              fontSize: '12px',
        ...
        >
          <p style={{ fontSize: '11px', fontWeight: '600', color: '#1D9E75', margin: '0 0 8px', letterSpacing: '0.5px' }}>
            {promo.business_name?.toUpperCase()}
          </p>
          <p style={{ fontSize: '20px', fontWeight: '700', color: '#0A2E1F', margin: '0 0 8px', lineHeight: '1.3' }}>
            {promo.headline}
          </p>
          {promo.subtext && (
            <p style={{ fontSize: '13px', color: '#666', margin: '0 0 14px', lineHeight: '1.6' }}>{promo.subtext}</p>
          )}
          {promo.cta_url && (
            <button
              onClick={handleClick}
        ...
        <button
          ...
          style={{
            width: '100%',
            background: seconds > 0 ? '#f5f5f5' : '#0A2E1F',
            color: seconds > 0 ? '#bbb' : 'white',
            border: 'none',
            padding: '14px',
            borderRadius: '10px',
            fontSize: '15px',
        ...
        <p style={{ fontSize: '11px', color: '#ccc', textAlign: 'center', margin: '10px 0 0' }}>
          Ads help keep FlushPin free for everyone
        </p>
      </div>
    </div>
  )
