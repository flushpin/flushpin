  ...
  return (
    <main className="fp-home">
      <style>{`
        ...
        .fp-hero-shell{z-index:1;grid-template-columns:1fr;align-items:center;gap:56px;width:min(1180px,100%);margin:0 auto;display:grid;position:relative}
        ...
        @media (min-width:981px){.fp-hero-shell{grid-template-columns:minmax(0,1.02fr) minmax(360px,.78fr)}}
        @media (max-width:900px){.fp-hero{min-height:auto;padding:58px 20px}.fp-business,.fp-footer-grid{grid-template-columns:1fr}.fp-phone{justify-self:center;max-width:min(390px,100%);margin:0 auto}.fp-feature-grid,.fp-stats{grid-template-columns:1fr}}
        @media (max-width:640px){.desktop-nav{display:none!important}.mobile-menu-btn{display:block!important}.fp-home-nav{padding:14px 18px!important}.fp-hero{padding:48px 20px 40px}.fp-hero-title{font-size:clamp(40px,10vw,56px)}.fp-hero-text{margin-bottom:24px;font-size:18px;line-height:1.55}.fp-section{width:min(100% - 28px,1120px);padding:58px 0}.fp-business{padding:24px}}
        @media (max-width:389px){.fp-home-logo img{height:44px!important;width:auto!important}.fp-hero{padding:42px 18px 34px}.fp-hero-title{font-size:clamp(36px,11vw,48px)}.fp-hero-text{font-size:17px}.fp-phone{display:none!important}.fp-search{flex-direction:column}.fp-search button{min-height:50px;width:100%}.fp-cta-row .fp-primary,.fp-cta-row .fp-secondary{width:100%}}
      `}</style>

      <nav className="fp-home-nav" style={{ background: 'white', borderBottom: '1px solid #f0f0f0', padding: '16px 20px', position: 'sticky', top: 0, zIndex: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, minWidth: 0 }}>
          <div className="fp-home-logo"><Logo height={58} /></div>
