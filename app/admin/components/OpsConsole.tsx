...
export default function OpsConsole({ accessToken, investorMode = false }: OpsConsoleProps) {
  ...
  const loadBusinessClaims = async () => {
    ...
    try {
      const res = await fetch('/admin/business-claims', { headers: authHeaders(accessToken) })
  ...
  const loadModeration = async () => {
    ...
    try {
      const res = await fetch('/admin/moderation', { headers: authHeaders(accessToken) })
      const json = await res.json().catch(() => null)
      if (!res.ok) {
  ...
  const loadLiveActivity = async () => {
    setLiveLoading(true)
    try {
      const res = await fetch('/admin/live?hours=24', { headers: authHeaders(accessToken) })
      if (res.ok) {
  ...
  useEffect(() => {
    if (activeTab !== 'live' || investorMode) return
  ...
  }, [activeTab, accessToken, investorMode])

  useEffect(() => {
    if (activeTab !== 'pins' || investorMode) return
    const initialLoad = window.setTimeout(() => void loadModeration(), 0)
    return () => window.clearTimeout(initialLoad)
  }, [activeTab, accessToken, investorMode])

  useEffect(() => {
    if (activeTab !== 'optout' || investorMode) return
  ...
  const loadData = async () => {
    ...
    const metricsError = metricsRes.ok
      ...
      : (metricsJson?.error as string | undefined) ||
        `Admin metrics unavailable (HTTP ${metricsRes.status}).`
  ...
  if (investorMode) {
    return (
      <div style={{ ...cardStyle(), padding: 28 }}>
        <h2 style={{ margin: 0, fontFamily: adminTheme.fontDisplay, fontSize: 20, color: adminTheme.text }}>
          Ops tools hidden in investor mode
        </h2>
        <p style={{ margin: '10px 0 0', color: adminTheme.textMuted, fontSize: 14, lineHeight: 1.5 }}>
          Moderation queues, live activity maps, and contact details are withheld during investor presentation mode.
        </p>
      </div>
    )
  }
  ...
  if (loading || !data) {
    return (
      <div style={{ ...shellStyle, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 240 }}>
        <div style={{ color: adminTheme.teal, fontSize: 16, fontFamily: adminTheme.fontDisplay }}>Loading ops…</div>
      </div>
    )
  }
  ...
  return (
    <div style={shellStyle}>
      ...
      >
        <div>
          <div style={{ fontFamily: adminTheme.fontDisplay, fontSize: 18, fontWeight: 700, color: adminTheme.teal }}>
            Operations
          </div>
          <div style={{ fontSize: 12, color: adminTheme.textMuted, marginTop: 2 }}>
            Moderation, claims, and live activity
          </div>
        </div>
        <button
          type="button"
      ...
      {data.metricsError ? (
        ...
        >
          ...
          <br />
          Member and activity counts are temporarily unavailable. Please try again shortly.
        </div>
      ) : null}
