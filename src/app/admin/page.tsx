...
const BAD_WORDS = [
  ...
  'sik', 'orospu', 'piç', 'göt', 'amk', 'bok', 'yarrak', 'sikiş'
]
    ...
    <div style={{ background: '#0f172a', minHeight: '100vh', color: '#e2e8f0', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ background: '#1e293b', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #334155', position: 'sticky', top: 0, zIndex: 100 }}>
        <div>
          ...
          <div style={{ fontSize: '11px', color: '#64748b' }}>John Ertan · Admin</div>
        </div>
      ...
      <div style={{ padding: '16px', maxWidth: '600px', margin: '0 auto' }}>
        ...
        {activeTab === 'overview' && (
          <div>
            ...
            <div style={{ background: '#1e293b', borderRadius: '12px', padding: '16px', border: '1px solid #334155' }}>
              <div style={{ fontWeight: 600, marginBottom: '12px', color: '#94a3b8' }}>🏙️ Top Cities</div>
              {cityStats.map((c, i) => (
        ...
        {activeTab === 'pins' && (
          <div>
            <div style={{ marginBottom: '12px', color: '#64748b', fontSize: '13px' }}>Son {recentPins.length} kayıt</div>
            {recentPins.map(pin => (
              <div key={pin.id} style={{ background: pin.isFlagged ? '#2d1b1b' : '#1e293b', borderRadius: '12px', padding: '14px', marginBottom: '10px', border: '1px solid ' + (pin.isFlagged ? '#ef4444' : '#334155') }}>
                <div style={{ fontWeight: 600, fontSize: '14px', color: pin.isFlagged ? '#fca5a5' : '#e2e8f0', marginBottom: '6px' }}>
                  {pin.name || 'Unknown'}
                  {pin.isFlagged && <span style={{ marginLeft: '8px', fontSize: '11px', background: '#ef4444', color: 'white', borderRadius: '4px', padding: '1px 6px' }}>⚠️ FLAGGED</span>}
                </div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button onClick={() => handleDelete(pin.id)} style={{ background: '#1e1b4b', color: '#a5b4fc', border: 'none', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', fontSize: '12px' }}>🗑️ Delete</button>
                </div>
              </div>
            ))}
          </div>
            ...
            {flaggedItems.length === 0
              ...
              : flaggedItems.map(item => (
                <div key={item.id} style={{ background: '#2d1b1b', borderRadius: '12px', padding: '14px', marginBottom: '10px', border: '1px solid #ef4444' }}>
                  <div style={{ fontWeight: 600, color: '#fca5a5', marginBottom: '6px' }}>⚠️ {item.content_type?.toUpperCase()} — {item.flag_reason}</div>
                  <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '4px' }}>Content: <span style={{ color: '#e2e8f0' }}>{item.content_text}</span></div>
        ...
        {activeTab === 'logs' && (
          <div>
            <div style={{ marginBottom: '12px', color: '#64748b', fontSize: '13px' }}>Son 50 admin aksiyonu</div>
            {adminLogs.length === 0
              ? <div style={{ textAlign: 'center', color: '#64748b', padding: '40px' }}>Henüz aksiyon yok</div>
              : adminLogs.map(log => (
                <div key={log.id} style={{ background: '#1e293b', borderRadius: '8px', padding: '12px', marginBottom: '8px', border: '1px solid #334155' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <span style={{ color: '#64748b', fontSize: '12px', marginLeft: '8px' }}>{log.target_type} #{log.target_id?.slice(0, 8)}</span>
                    </div>
